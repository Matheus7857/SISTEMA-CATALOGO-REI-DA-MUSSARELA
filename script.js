...
async function salvarFirebase() {
  ...
  try {
    ...
    // Limpar tudo em paralelo (produto_tabelas primeiro por FK)
    await limparTabelaSupabase("produto_tabelas");
    await Promise.all([
      limparTabelaSupabase("produtos"),
      limparTabelaSupabase("categorias"),
      limparTabelaSupabase("tabelas_comerciais")
    ]);

    // Salvar empresa + categorias + tabelas em paralelo, depois produtos, depois produto_tabelas
    await Promise.all([
      inserirSupabase("empresa", {
        id: 1,
      ...
      inserirSupabase("categorias", categoriasFormatadas, "nome"),
      inserirSupabase("tabelas_comerciais", tabelasFormatadas, "id")
    ]);
...
async function carregarFirebase() {
  ...
  // Agora os dados principais vêm do Supabase — carregamento paralelo para maior velocidade.
  try {
    const [empresaBanco, categoriasBanco, tabelasBanco, produtosBanco, produtoTabelasBanco] =
      await Promise.all([
        supabaseFetch("empresa",           { query: "?id=eq.1&select=*",           prefer: "return=representation" }),
        supabaseFetch("categorias",        { query: "?select=*&order=nome.asc",    prefer: "return=representation" }),
        supabaseFetch("tabelas_comerciais",{ query: "?select=*&order=nome.asc",    prefer: "return=representation" }),
        supabaseFetch("produtos",          { query: "?select=*&order=nome.asc",    prefer: "return=representation" }),
        supabaseFetch("produto_tabelas",   { query: "?select=*",                   prefer: "return=representation" })
      ]);
...
atualizarHora();
// Sincroniza com a virada do minuto para economizar CPU
const _msAteProximoMinuto = (60 - new Date().getSeconds()) * 1000;
setTimeout(() => { atualizarHora(); setInterval(atualizarHora, 60000); }, _msAteProximoMinuto);
...
async function salvarProdutoFinal(id, codigo, nome, categoria, ativo, descricao, tabelas, imagem) {
  const adminUid = usuarioAtual ? usuarioAtual.uid : "admin";
  const dataAgora = new Date().toISOString();

  if (id) {
    const produtoAntes = produtos.find(p => String(p.id) === String(id));
    let mudancasPreco = [];
    if (produtoAntes) {
      tabelas.forEach(novaTabela => {
        const tabelaAntiga = (produtoAntes.tabelas || []).find(t => paraMaiusculo(t.nomeTabela) === paraMaiusculo(novaTabela.nomeTabela));
        if (tabelaAntiga) {
          if (tabelaAntiga.valor !== novaTabela.valor) {
            mudancasPreco.push({
              tabela: novaTabela.nomeTabela,
              valorAntigo: tabelaAntiga.valor,
              valorNovo: novaTabela.valor
            });
          }
        } else {
          mudancasPreco.push({
    ...
    produtos = produtos.map(p => {
      if (String(p.id) === String(id)) {
        return { ...p, codigo, nome, categoria, ativo, descricao, tabelas, imagem, atualizadoEm: dataAgora };
      }
      return p;
    });

    if (mudancasPreco.length > 0) {
      for (const mudanca of mudancasPreco) {
        try {
          await addDoc(collection(db, "notificacoes"), {
            tipo: "preco_atualizado",
            titulo: "💰 PREÇO ATUALIZADO",
            conteudo: `O PRODUTO ${nome} TEVE SEU PREÇO ALTERADO NA TABELA ${mudanca.tabela} PARA R$ ${formatarValor(mudanca.valorNovo)}.`,
            criado_em: dataAgora,
            criado_por: adminUid
          });
        } catch (e) {
          console.error("Erro ao gerar notificacao de preco:", e);
        }
      }
    }
  } else {
    ...
    try {
      await addDoc(collection(db, "notificacoes"), {
        ...
        conteudo: `O PRODUTO ${nome} FOI ADICIONADO NA CATEGORIA ${categoria}.`,
        criado_em: dataAgora,
        criado_por: adminUid
      });
    } catch (e) {
      console.error("Erro ao gerar notificacao de novo produto:", e);
    }
  }
...
async function duplicarProduto(id) {
  ...
  try {
    await addDoc(collection(db, "notificacoes"), {
      tipo: "novo_produto",
      titulo: "🆕 NOVO PRODUTO CADASTRADO",
      conteudo: `O PRODUTO ${novoProduto.nome} FOI ADICIONADO NA CATEGORIA ${novoProduto.categoria} (DUPLICADO).`,
      criado_em: dataAgora,
      criado_por: adminUid
    });
  } catch (e) {
    console.error("Erro ao gerar notificacao de duplicacao:", e);
  }
    ...
    <div class="card-produto ${produto.ativo ? "" : "inativo"}">
      <div class="card-produto-img-wrap" onclick="abrirPopupProduto(${produto.id})" title="VER VALORES">
        <img src="${produto.imagem || imagemPadrao()}" alt="${produto.nome}" loading="lazy"
             onerror="this.src='${imagemPadrao()}'; this.onerror=null;">
...
function debouncePesquisa() {
  ...
  _debounceTimer = setTimeout(renderizarCatalogo, 280);
}
...
window.addEventListener("beforeinstallprompt", e => {
  e.preventDefault();
  _pwaPrompt = e;
  // Mostrar pulsação no botão quando PWA instalável
...
window.addEventListener("appinstalled", () => {
  _pwaPrompt = null;
  toast("APP INSTALADO COM SUCESSO!", "sucesso");
});

function abrirModalInstalarApp() {
  // Verificar se APK existe
  fetch("catalogo-erp.apk", { method: "HEAD" })
    .then(r => {
      const btnAPK   = document.getElementById("btnDownloadAPK");
      const txtAPK   = document.getElementById("apkStatusTexto");
      const semAPK   = document.getElementById("apkIndisponivel");
      if (r.ok) {
        if (btnAPK)  btnAPK.style.display  = "";
        if (semAPK)  semAPK.style.display  = "none";
        if (txtAPK)  txtAPK.textContent    = "Arquivo APK disponível para download.";
      } else {
    ...
    .catch(() => {
      ...
      if (semAPK) semAPK.style.display  = "";
      if (txtAPK) txtAPK.textContent    = "APK ainda não gerado pelo administrador.";
    });

  // Mostrar/ocultar botão PWA conforme disponibilidade
  const btnPWA   = document.getElementById("btnPWAInstall");
  const opcaoPWA = document.getElementById("opcaoPWA");
  if (_pwaPrompt) {
    if (btnPWA)   btnPWA.style.display   = "";
    if (opcaoPWA) opcaoPWA.style.opacity = "1";
  } else {
    if (btnPWA)   btnPWA.style.display   = "none";
    if (opcaoPWA) opcaoPWA.style.opacity = "0.5";
  }

  document.getElementById("modalInstalarApp").classList.add("ativo");
  fecharMenuMobileDepoisClique();
}

function fecharModalInstalarApp() {
  document.getElementById("modalInstalarApp").classList.remove("ativo");
}

async function instalarPWA() {
  if (!_pwaPrompt) {
    toast("USE O CHROME E ACESSE PELO CELULAR PARA INSTALAR.", "info");
    return;
  }
  _pwaPrompt.prompt();
...
function toggleCentralNotificacoes() {
  const painel = document.getElementById("notificacaoPainel");
  if (!painel) return;
  const aberto = painel.classList.toggle("ativo");
  if (aberto) {
    setTimeout(() => document.addEventListener("click", _fecharNotifFora), 10);
  } else {
    document.removeEventListener("click", _fecharNotifFora);
  }
}
...
function _getIdsLidos() {
  try { return JSON.parse(localStorage.getItem("notif_lidas") || "[]"); }
  catch { return []; }
}

function marcarNotificacaoComoLida(id) {
  const lidas = _getIdsLidos();
...
function marcarTodasComoLidas() {
  const items = document.querySelectorAll(".notificacao-item[data-id]");
  const lidas = _getIdsLidos();
  items.forEach(el => {
    const id = el.dataset.id;
    if (id && !lidas.includes(id)) lidas.push(id);
...
function _iconeNotif(tipo) {
  if (tipo === "preco_atualizado") return "fa-tag";
  if (tipo === "novo_produto")     return "fa-box-open";
  return "fa-bell";
}

function atualizarBadgeNotificacoes() {
  const badge    = document.getElementById("notificacaoBadge");
  const naoLidas = document.querySelectorAll(".notificacao-item.nao-lido").length;
  if (!badge) return;
  if (naoLidas > 0) {
...
function _renderizarNotificacoes(notifs) {
  ...
  lista.innerHTML = notifs.map(n => {
    ...
    const icone = _iconeNotif(n.tipo);
    return `
      <div class="notificacao-item ${lido ? "" : "nao-lido"}" data-id="${n.id}"
           onclick="marcarNotificacaoComoLida('${n.id}'); this.classList.remove('nao-lido'); atualizarBadgeNotificacoes();">
        <div class="notificacao-item-icone">
          <i class="fa-solid ${icone}"></i>
        </div>
        <div class="notificacao-item-corpo">
          <span class="notificacao-item-titulo">${n.titulo || "NOTIFICAÇÃO"}</span>
          <span class="notificacao-item-conteudo">${n.conteudo || ""}</span>
          <span class="notificacao-item-tempo">
            <i class="fa-regular fa-clock"></i>
            ${_tempoRelativo(n.criado_em)}
          </span>
        </div>
      </div>`;
  }).join("");
...
function iniciarNotificacoes() {
  ...
  unsubscribeNotificacoes = onSnapshot(q, snap => {
    ...
    _renderizarNotificacoes(notifs);
    ...
    if (!primeiraLeitura) {
      snap.docChanges().forEach(change => {
        ...
        const lidas = _getIdsLidos();
        if (lidas.includes(change.doc.id)) return;
        const idade = Date.now() - new Date(n.criado_em).getTime();
        if (idade < 10000) {
          toast(n.titulo || "NOVA NOTIFICAÇÃO", "info");
          const sino = document.querySelector(".btn-notificacao i");
          if (sino) {
            sino.classList.add("sino-wobble");
            setTimeout(() => sino.classList.remove("sino-wobble"), 700);
          }
        }
      });
    }
  ...
  }, err => console.warn("Notificações:", err.message));
}
...
window.debouncePesquisa       = debouncePesquisa;
window.abrirModalInstalarApp  = abrirModalInstalarApp;
window.fecharModalInstalarApp = fecharModalInstalarApp;
window.instalarPWA            = instalarPWA;

window.toggleCentralNotificacoes  = toggleCentralNotificacoes;
